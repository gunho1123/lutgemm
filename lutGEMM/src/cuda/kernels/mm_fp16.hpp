    ...
    for(int m = m_start;m < m_end;m += m_step){
        ...
        for(int b=0;b < NUM_BITS;b++){
            ...
            for(int kt=0;kt < K_TILE_SIZE/32;kt++){
                uint32_t reg_w = bW[blockIdx.z * K/32 * NUM_BITS * M + kt * NUM_BITS * M + b * M + m]; 
                ...
                reg_w = bW[blockIdx.z * K/32 * NUM_BITS * M + kt * NUM_BITS * M + b * M + m + 1]; 
        ...
        atomicAdd((half2*)&output[blockIdx.z * M + m], __halves2half2(reg_o0, reg_o1));
    }
}
